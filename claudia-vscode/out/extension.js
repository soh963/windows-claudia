...
function activate(context) {
    ...
    vscode.window.showInformationMessage('Claudia VS Code extension loaded! Use Ctrl+Shift+C to open Claudia panel.', 'Open Claudia').then(selection => {
...
function createClaudiaPanel(context, initialData) {
    ...
    claudiaPanel = vscode.window.createWebviewPanel('claudia', 'Claudia - Claude AI Assistant', vscode.ViewColumn.One, {
        enableScripts: true,
        localResourceRoots: [
    ...
    claudiaPanel.webview.onDidReceiveMessage(message => {
...
function getWebviewContent(context, webview) {
    ...
    htmlContent = htmlContent.replace(/(\/assets\/[^"'\s>]+)/g, (match) => `${baseUri}${match}`);
...
async function readFileForWebview(filePath) {
    try {
